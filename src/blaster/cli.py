...
from blaster.stats import gaussian_heuristic, get_profile, rhf, slope
...
def __main__():
    ...
    parser.add_argument("--cores", "-j", type=int, default=cpu_count() // 2, help="number of cores to be used")
    ...
    parser.add_argument("--quiet", "-q", action="store_true", help="Quiet mode will not output the output basis")
    parser.add_argument(
        "--anim",
        "-a",
        type=str,
    ...
    # LLL parameters
    parser.add_argument("--delta", type=float, default=0.99, help="delta factor for Lovasz condition")
    parser.add_argument(
        "--lll_size",
        "-L",
        type=int,
    ...
    parser.add_argument("--beta", "-b", type=int, help="Blocksize used within BKZ. 0 if not desired.")
    parser.add_argument("--bkz-tours", "-t", type=int, default=8, help="Number of BKZ-tours to perform.")
    parser.add_argument(
        "--bkz-size",
        "-S",
        type=int,
    ...
    parser.add_argument("--bkz-prog", "-P", type=int, help="Progressive block size increment for BKZ.")
    ...
    if args.verbose:
        ...
        print(
            f"E[∥b₁∥] ~ {norm_b1:.2f} {comparison}(GH: λ₁ ~ {gaussian_heuristic(prof):.2f})",
            file=stderr,
        )

    # Multithreading is used in two places:
    # 1) Matrix multiplication (controlled by Eigen)
    # 2) Block reduction inside the nanobind/C++ core (parallelized with OpenMP)
